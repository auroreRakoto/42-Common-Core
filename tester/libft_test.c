...
/* --- simple C-like unescape: \n \t \r \f \v \\ \" \' \xHH --- */
static char *unescape(const char *s)
{
	size_t n = strlen(s);
	char *out = (char*)malloc(n + 1); /* output won't be longer than input */
	if (!out) return NULL;
	size_t i = 0, j = 0;
	while (i < n) {
		if (s[i] == '\\' && i + 1 < n) {
...
int main(int argc, char **argv)
{
	...
	{
		size_t nfields=0;
		char **f = split_fields(arg,&nfields); /* "STRING|CHAR" */
		if (!f || nfields != 2)
		{
			puts("null");
			if(f)
	...
	if (strcmp(fn, "ft_strlcpy") == 0)
	{
		...
		/* Toujours allouer au moins 1 octet et NUL-terminer avant l'appel,
		même quand size == 0 (ft_strlcpy n’écrit rien dans ce cas). */
		size_t cap = (size_t)(size ? size : 1);
		...
		buf[0] = '\0';  /* <<< important: éviter l’affichage de garbage pour size==0 */
